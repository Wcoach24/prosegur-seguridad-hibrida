...
// Mobile: single class on container → CSS animation-delay handles stagger
// Desktop: per-word setTimeout (original behavior)
// ════════════════════════════════════════════════
(function () {
  'use strict';
  ...
  const textObs = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        if (isMobile) {
          // ONE DOM mutation → browser handles all animation timing on compositor
...
// ════════════════════════════════════════════════
// MOBILE SLIDER AFFORDANCE — Dots + Auto-nudge
// ════════════════════════════════════════════════
(function () {
  'use strict';
  if (!window.matchMedia('(max-width: 768px)').matches) return;

  function setupSlider(container, cardSelector) {
    if (!container) return;
    ...
    container.addEventListener('scroll', function () {
      ...
      scrollTimer = setTimeout(function () {
        var gap = parseFloat(getComputedStyle(container).gap) || 16;
        var cardW = cards[0].offsetWidth + gap;
        var idx = Math.round(container.scrollLeft / cardW);
    ...
    // — Auto-nudge: gentle scroll hint on first view —
    var nudgeObs = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          nudgeObs.unobserve(container);
        }
      });
    }, { threshold: 0.3 });
    nudgeObs.observe(container);
  }

  setupSlider(document.querySelector('.eco-grid'), '.eco-card');
  setupSlider(document.querySelector('.sector-cards'), '.sector-card');
})();
